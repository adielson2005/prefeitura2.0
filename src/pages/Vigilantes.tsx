...
import { Shield, UserPlus, Filter, Download, UserCheck, CalendarOff, AlertTriangle, X, Save } from "lucide-react";
import { useState, useEffect } from "react";
import { dataService } from "@/lib/dataService";

export default function Vigilantes() {
  const [vigilantes, setVigilantes] = useState(dataService.getProfessionalsByCategory('VIGILANTE'));
  ...
  const handleExport = async () => {
    ...
    const csv = filteredVigilantes.map(v => `"${v.name}","${v.area}","${v.status}"`).join('\n');
  ...
  const stats = {
    ...
    problemas: vigilantes.filter(v => v.status === 'ATRASADO' || v.status === 'AUSENTE').length,
  };
        ...
        {showNewModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 max-w-md w-full border border-slate-700/50 shadow-2xl">
              <h3 className="text-lg font-bold text-white mb-5">Novo Vigilante</h3>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleCreateVigilante()}
                  />
                </div>
        ...
        {showFilterModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 max-w-md w-full border border-slate-700/50 shadow-2xl">
              ...
              <p className="text-sm text-slate-400 mb-4">Encontrados: <span className="text-white font-semibold">{filteredVigilantes.length}</span> vigilante(s)</p>
