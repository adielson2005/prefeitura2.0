...
export default function Vigilantes() {
  const [vigilantes, setVigilantes] = useState(
    dataService.getProfessionalsByCategory("VIGILANTE")
  );
  ...
  useEffect(() => {
    const unsubscribe = dataService.subscribe(() => {
      setVigilantes(dataService.getProfessionalsByCategory("VIGILANTE"));
    });
    return unsubscribe;
  }, []);
  ...
  const handleExport = async () => {
    ...
    const csv = filteredVigilantes
      .map((v) => `"${v.name}","${v.area}","${v.status}"`)
      .join("\n");
  ...
  const getFilteredVigilantes = () => {
    let result = vigilantes;
    if (filterText) {
  ...
  const filteredVigilantes = getFilteredVigilantes();
  const stats = {
    total: vigilantes.length,
    em_servico: vigilantes.filter((v) => v.status === "EM_SERVICO").length,
    folga: vigilantes.filter((v) => v.status === "FOLGA").length,
    problemas: vigilantes.filter(
  ...
  return (
    ...
    >
      <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && handleCreateVigilante()
                    }
                  />
                </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="ml-auto text-sm opacity-60">
                          {vigilantes.filter((v) => v.status === status).length}
                        </span>
                      </div>
                    </button>
                  )
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 xs:p-3 sm:p-4">
            <div className="bg-gradient-to-br from-slate-800 via-slate-850 to-slate-900 rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 md:p-8 max-w-lg w-full border border-slate-700/50 shadow-2xl max-h-[96vh] sm:max-h-[95vh] overflow-y-auto">
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mb-3 xs:mb-4 sm:mb-6 pb-3 xs:pb-4 sm:pb-6 border-b border-slate-700/50">
                <div className="h-14 w-14 xs:h-16 xs:w-16 sm:h-20 sm:w-20 flex-shrink-0 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-white text-lg xs:text-xl sm:text-2xl font-bold shadow-lg">
                ...
                <div className="flex-1 text-center sm:text-left w-full min-w-0">
                  ...
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 xs:gap-2">
                    <span className="px-2 xs:px-2.5 sm:px-3 py-0.5 xs:py-1 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-full text-[10px] xs:text-xs font-semibold whitespace-nowrap">
        ...
        {/* Table */}
        <ProfessionalTable
          professionals={filteredVigilantes}
