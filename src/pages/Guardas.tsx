...
export default function Guardas() {
  const [guardas, setGuardas] = useState(
    dataService.getProfessionalsByCategory("GUARDA")
  );
  ...
  useEffect(() => {
    const unsubscribe = dataService.subscribe(() => {
      setGuardas(dataService.getProfessionalsByCategory("GUARDA"));
    });
    return unsubscribe;
  }, []);

  const handleCreateGuarda = async () => {
  ...
  const handleExport = async () => {
    ...
    const csv = filteredGuardas
      .map((g) => `"${g.name}","${g.area}","${g.status}"`)
      .join("\n");
  ...
  const filteredGuardas = getFilteredGuardas();
  const stats = {
    total: guardas.length,
    em_servico: guardas.filter((g) => g.status === "EM_SERVICO").length,
    folga: guardas.filter((g) => g.status === "FOLGA").length,
    problemas: guardas.filter(
      (g) => g.status === "ATRASADO" || g.status === "AUSENTE"
    ).length,
  };
  ...
  return (
    <AppLayout title="Guardas" subtitle="Gestão de guardas municipais">
        ...
        {showNewModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 max-w-md w-full border border-slate-700/50 shadow-2xl">
              <h3 className="text-lg font-bold text-white mb-5">Novo Guarda</h3>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onKeyDown={(e) => e.key === "Enter" && handleCreateGuarda()}
                  />
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option>Terminal Rodoviário</option>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="ml-auto text-sm opacity-60">
                          {guardas.filter((g) => g.status === status).length}
                        </span>
                      </div>
                    </button>
                  )
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 xs:p-3 sm:p-4">
            <div className="bg-gradient-to-br from-slate-800 via-slate-850 to-slate-900 rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 md:p-8 max-w-lg w-full border border-slate-700/50 shadow-2xl max-h-[96vh] sm:max-h-[95vh] overflow-y-auto">
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mb-3 xs:mb-4 sm:mb-6 pb-3 xs:pb-4 sm:pb-6 border-b border-slate-700/50">
                <div className="h-14 w-14 xs:h-16 xs:w-16 sm:h-20 sm:w-20 flex-shrink-0 rounded-full bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center text-white text-lg xs:text-xl sm:text-2xl font-bold shadow-lg">
                ...
                <div className="flex-1 text-center sm:text-left w-full min-w-0">
                  ...
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 xs:gap-2">
                    <span className="px-2 xs:px-2.5 sm:px-3 py-0.5 xs:py-1 bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-full text-[10px] xs:text-xs font-semibold whitespace-nowrap">
