...
import { Users, UserPlus, Filter, Download, UserCheck, CalendarOff, AlertTriangle, X, Save } from "lucide-react";
import { useState, useEffect } from "react";
import { dataService } from "@/lib/dataService";

export default function Guardas() {
  const [guardas, setGuardas] = useState(dataService.getProfessionalsByCategory('GUARDA'));
  ...
  const handleExport = async () => {
    ...
    const csv = filteredGuardas.map(g => `"${g.name}","${g.area}","${g.status}"`).join('\n');
  ...
  const getFilteredGuardas = () => {
    let result = guardas;
    if (filterText) {
      result = result.filter(g => g.name.toLowerCase().includes(filterText.toLowerCase()));
    }
    if (filterStatus) {
  ...
  const stats = {
    ...
    problemas: guardas.filter(g => g.status === 'ATRASADO' || g.status === 'AUSENTE').length,
  };
        ...
        {showFilterModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 max-w-md w-full border border-slate-700/50 shadow-2xl">
              ...
              <p className="text-sm text-slate-400 mb-4">Encontrados: <span className="text-white font-semibold">{filteredGuardas.length}</span> guarda(s)</p>
